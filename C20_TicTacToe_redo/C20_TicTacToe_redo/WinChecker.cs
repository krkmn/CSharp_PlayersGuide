...
namespace C20_TicTacToe_redo
{
    public class WinChecker
    {
        ...
        private bool CheckForWin(Board board, State player)
        {
            ...
            {
                if (AreAll(board, new Position[] { new Position(row, 0), new Position(row, 1), new Position(row, 2) },
                    player)) return true;
            }

            for (int column = 0; column < 3; column++)
            {
                if (AreAll(board, new Position[] { new Position(0, column), new Position(1, column), new Position(2, column) },
                    player)) return true;
            }

            if (AreAll(board, new Position[] { new Position(0, 0), new Position(1, 1), new Position(2, 2) },
             player)) return true;

            if (AreAll(board, new Position[] { new Position(0, 2), new Position(1, 1), new Position(2, 0) },
             player)) return true;
